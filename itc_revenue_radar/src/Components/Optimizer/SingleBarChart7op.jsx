...
function SingleBarChart7op({ optimizeddatasetincrement, type, isValue, range, maxValuedynamicVolume }) {
  ...
  if (isStacked) {
    ...
    filtered?.forEach(item => {
      if (!groupedData[item.variable_name]) {
        groupedData[item.variable_name] = {};
      }
      groupedData[item.variable_name][item.frequency] =
    ...
    // Non-stacked chart
    categories = filtered?.map(item => item.variable_name);
    series = [{
      name: "ROI",
...
export default SingleBarChart7op;
