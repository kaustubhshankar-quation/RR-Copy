...
function ModelPerformance() {
  ...
  return (
    <>
          ...
          <div className="modal show" style={{ display: 'block', position: 'fixed', top: 0, left: sidebarOffset, width: `calc(100vw - ${sidebarOffset}px)`, height: '100vh', zIndex: 9999, overflowX: 'hidden', overflowY: 'auto' }} tabIndex="-1">
            <div className="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable" style={{ maxWidth: '90vw', marginLeft: 'auto', marginRight: 'auto' }}>
              <div className="modal-content border-0 shadow" style={{ background: 'var(--rr-bg-panel)' }}>
                <div className="modal-header" style={{ background: 'var(--rr-bg-soft)', borderBottom: '1px solid var(--rr-border)', padding: '12px 20px' }}>
                  ...
                  <div className="d-flex align-items-center gap-2">
                    {exportcontributiondata?.length > 0 && (
                      <button className="mp-secondary-btn" onClick={downloadLineChartResults}>
                  ...
                  {loader2 ? (
                    ...
                    <>
                      {mapetable?.find(t => t?.market === modalRow?.market) && (
                        <div className="mp-chart-card mb-3">
                      ...
                      <div className="container-fluid px-0">
                        <div className="row g-3">
                          {selectedmarketdataset?.plot2?.length > 0 && (
                            <div className="col-lg-6 col-md-12">
                              <div className="mp-mini-card h-100">
                                <PieCharts isValue={isValue} range={`${formatDate(fromrangeonplots)}-${formatDate(torangeonplots)}`} fulldataset={selectedmarketdataset} type={modalRow?.market} displaynames={displaynames} />
                              </div>
                            </div>
                          )}
                          ...
                          {selectedmarketdataset?.plot4?.length > 0 && (
                            <div className="col-lg-6 col-md-12">
                              <div className="mp-mini-card h-100">
                                <SingleBarChart2 fulldataset={selectedmarketdataset} type={modalRow?.market} isValue={isValue} range={`${formatDate(fromrangeonplots)}-${formatDate(torangeonplots)}`} maxValuedynamicVolume={Math.ceil(Math.max(...selectedmarketdataset?.plot4.map(it=>it.contribution))/100)*100} maxValuedynamicValue={Math.ceil(Math.max(...selectedmarketdataset?.plot4.map(it=>it.contribution_sales_value/100000))/100)*100} />
                              </div>
                            </div>
                          )}
                          {selectedmarketdataset?.plot10?.length > 0 && (
                            <div className="col-lg-6 col-md-12">
                              <div className="mp-mini-card h-100">
                                <SingleBarChart5 fulldataset={selectedmarketdataset} type={modalRow?.market} isValue={isValue} range={`${formatDate(fromrangeonplots)}-${formatDate(torangeonplots)}`} maxValuedynamicVolume={Math.ceil(Math.max(...selectedmarketdataset?.plot10.map(it=>it.contribution))/100)*100} maxValuedynamicValue={Math.ceil(Math.max(...selectedmarketdataset?.plot10.map(it=>it.contribution_sales_value/100000))/100)*100} />
                              </div>
                            </div>
                          )}
                          {selectedmarketdataset?.plot5?.length > 0 && (
                            <div className="col-lg-6 col-md-12">
                              <div className="mp-mini-card h-100">
                                <SingleBarChart4 fulldataset={selectedmarketdataset} type={modalRow?.market} isValue={isValue} range={`${formatDate(fromrangeonplots)}-${formatDate(torangeonplots)}`} maxValuedynamicVolume={Math.ceil(Math.max(...selectedmarketdataset?.plot5?.map(it=>Number(it?.attribute_value_per_roi))))} />
                              </div>
                            </div>
                          )}
                          {selectedmarketdataset?.plot13?.length > 0 && (
                            <div className="col-lg-6 col-md-12">
                              <div className="mp-mini-card h-100">
                                <SingleBarChart8 fulldataset={selectedmarketdataset} type={modalRow?.market} isValue={isValue} range={`${formatDate(fromrangeonplots)}-${formatDate(torangeonplots)}`} maxValuedynamicVolume={(Math.ceil(Math.max(...selectedmarketdataset?.plot13?.map(it=>Number(it?.attribute_value_per_roi))))/10)*10} />
                              </div>
                            </div>
                          )}
                          {selectedmarketdataset?.plot12?.length > 0 && (
                            <div className="col-lg-6 col-md-12">
                              <div className="mp-mini-card h-100">
                                <SingleBarChart7 fulldataset={selectedmarketdataset} type={modalRow?.market} isValue={isValue} range={`${formatDate(fromrangeonplots)}-${formatDate(torangeonplots)}`} maxValuedynamicVolume={Math.ceil(Math.max(...selectedmarketdataset?.plot12?.map(it=>Number(it?.attribute_value_per_roi))))} />
                              </div>
                            </div>
                          )}
                          {selectedmarketdataset?.plot11?.length > 0 && (
                            <div className="col-lg-6 col-md-12">
                              <div className="mp-mini-card h-100">
                                <SingleBarChart6 fulldataset={selectedmarketdataset} type={modalRow?.market} isValue={isValue} range={`${formatDate(fromrangeonplots)}-${formatDate(torangeonplots)}`} maxValuedynamicVolume={Math.ceil(Math.max(...selectedmarketdataset?.plot11?.map(it=>Number(it?.effectiveness)))/10)*10} />
                              </div>
                            </div>
                          )}
                          {selectedmarketdataset?.plot9?.length > 0 && (
                            <div className="col-lg-12 col-md-12">
                              <div className="mp-mini-card h-100">
                                <SingleBarChart3 fulldataset={selectedmarketdataset} type={modalRow?.market} isValue={isValue} range={`${formatDate(fromrangeonplots)}-${formatDate(torangeonplots)}`} maxValuedynamicVolume={Math.ceil(Math.max(...selectedmarketdataset?.plot9?.map(it=>Number(it?.effectiveness)))/10)*10} />
                              </div>
                            </div>
                          )}
                        </div>
      ...
      .dark-theme .mp-select__control:hover {
        border-color: rgba(23, 162, 184, 0.40) !important;
      }

      .dark-theme .mp-select__control--is-focused {
        border-color: #17A2B8 !important;
        box-shadow: 0 0 0 2px rgba(23, 162, 184, 0.15) !important;
      }

      .dark-theme .mp-select__option--is-focused {
        background: rgba(23, 162, 184, 0.08) !important;
      }

      .dark-theme .mp-select__option--is-selected {
        background: rgba(23, 162, 184, 0.15) !important;
        color: #17A2B8 !important;
      }

      .dark-theme .mp-select__multi-value {
